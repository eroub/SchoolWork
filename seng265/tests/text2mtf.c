...
#define BUFLEN 120*20
int num_words, num_dic_words, count;
char word_list[MAX_WORD][MAX_LENGTH], basis[MAX_WORD][MAX_LENGTH], final[BUFLEN][BUFLEN];
...
void tokenize(FILE *infile){
	...
	while(fgets(input, BUFLEN, infile) != NULL){
		...
		token = strtok(input, " ");
		while(token != NULL){
			if(num_words < MAX_WORD) strncpy(basis[num_words++], token, MAX_LENGTH);
			token = strtok(NULL, " ");
		}
	}
}
